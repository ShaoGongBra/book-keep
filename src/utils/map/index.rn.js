...
export default class RoutePlan {
  ...
  static openMap = (pos) => {
    const startMap = type => {
      switch (type) {
        case 'baidu':
          ...
          break
        case 'qq':
          const qq = bdDecrypt(pos.latitude, pos.longitude)
          Linking.openURL(`qqmap://map/marker?marker=coord:${qq.lat},${qq.lon};title:${pos.name};addr:${pos.address}&referer=${config.qqmapKey}`)
          break
        case 'amap':
    ...
    return Promise.all([RoutePlan.isInstallAmap(), RoutePlan.isInstallBaiDuMap(), RoutePlan.isInstallQQMap()]).then(res => {
      const list = [[],[]]
      const maps = [
        {
          title: '高德地图',
